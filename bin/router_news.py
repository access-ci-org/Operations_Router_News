...
class Router():
    def __init__(self):
        ...
        parser.add_argument('-c', '--config', action='store', default='./router_news.conf', \
                            help='Configuration file default=./router_news.conf')
...
#   2023-02-16 JP - figure out later the appropriate level of ssl verification
    ...
    def Warehouse_Operations_News(self, input_obj):
        ...
        for p_res in input_obj:
            URN = self.NEWSURNPREFIX + p_res['outage_id']
            try:
                news_item, created = News.objects.update_or_create(
                                    ...
                                    defaults = {
                                        ...
                                        'NewsStart': parse_datetime(p_res['start_timestamp']),
                                        'NewsEnd': parse_datetime(p_res['end_timestamp']),
                                        'NewsType': p_res['type'],
                                        'DistributionOptions': p_res['distribution_options'],
            ...
            for resource in p_res['affected_infrastructure']:
                try:
                    assoc_type = 'Resource'
                    assoc_id = resource['infra_resourceid']
